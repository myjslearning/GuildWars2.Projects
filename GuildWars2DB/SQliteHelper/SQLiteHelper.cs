...
namespace System.Data.SQLite
{
    ...
    internal class SQLiteHelper
    {
        ...
        public bool IsTableCreated(GW2Entities table) => IsTableCreated(GuildWars2DBDictionary.EntityToName(table));

        public DataTable GetColumnStatus(string tableName) => Select($"PRAGMA table_info(`{tableName}`);");
        ...
        public DataTable SelectAll(GW2Entities table) => Select($"Select * from {GuildWars2DBDictionary.EntityToName(table)} order by ID;");
        ...
        public void Insert(GW2Entities table, Dictionary<string, object> dic) {
            string tableName = GuildWars2DBDictionary.EntityToName(table);
            if(tableName != null) {
                Insert(tableName, dic);
            }
        }
        ...
        public void Update(GW2Entities table, Dictionary<string, object> dic, string colCond, object varCond) {
            string tableName = GuildWars2DBDictionary.EntityToName(table);
            if(tableName != null) {
                Update(tableName, dic, colCond, varCond);
            }
        }
        ...
        public int Count(GW2Entities table, string conditionName, object conditionValue) => Count(GuildWars2DBDictionary.EntityToName(table), conditionName, conditionValue);

        public int Count(string tableName, string conditionName, object conditionValue) {
            cmd.CommandText = string.Format("SELECT count(*) FROM {0} WHERE {1}='{2}'", tableName, conditionName, conditionValue);
